...
start.addEventListener('click', ()=> {
    ...
    timerStart = true;
    if (minuteS != 0 && minutesSession.textContent != 0 && minuteB != 0 && minutesBreak.textContent != 0) {
    interval = setInterval(startTimer, 10)
    }

    if (minuteS == 0 && minutesSession.textContent == 0  && timerStart === false) {
        modalWindow.classList.add('show');
        modalWindow.classList.remove('hide');
        createModalMessage('Session')
    }

    if (minuteB == 0 && minutesBreak.textContent == 0  && timerStart === false) {
